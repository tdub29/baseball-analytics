...
4. **Battle calc**  
   ...
   - Reads `real5_pbp_baseballr_style.csv` and `real5_selected_games.csv` from **this folder** (`battles/`).  
...
| **real5_pbp_baseballr_style.csv** | Combined PBP for “real5” games (contest IDs you chose). |
| **real5_selected_games.csv** | contest_id, away_short, home_short. |
| **real5_battle_calc_output.txt** | Human-readable battle report: per game, all 10 metrics and goal met Y/N. |
| **battle_pdf/generate-pdf.mjs** | Node: reads battle output txt, writes `season_battle_report.pdf` and per-game `{date}_{opponent}_{id}_Battle_report.pdf`. Auto-run by fetch/calc. |
...
- **Re-fetch all current games and recalc (Playwright):**  
  ...
  (A browser window will open for each URL; team names come from the page. Then writes text report and Node PDFs: `battle_pdf/season_battle_report.pdf` + per-game PDFs with readable names.)

- **Recalc only (no fetch):**  
  Ensure `real5_pbp_baseballr_style.csv` and `real5_selected_games.csv` are in `battles/`, then from `battles/`:  
  `python baseballr_battle_calc.py`  
  (Writes text report and runs Node PDF step: `season_battle_report.pdf` + per-game in `battle_pdf/`.)
...
- **Battle calc** expects to be run from `battles/` with the two CSVs in the same directory; it uses non-relative imports (`baseballr_description_mappings`, `battle_logic`) so it can be executed as a script.
